...
class AppointmentForm(forms.ModelForm):
    ...
    def clean_duration(self):
        ...
        if isinstance(duration, timedelta):
            return duration  # Already a timedelta, no need to convert
        if duration:
            return timedelta(minutes=int(duration))  # Convert minutes to timedelta
        return duration
    ...
    def clean(self):
        ...
        time_str = cleaned_data.get('time')  # Start time as a string
        duration = cleaned_data.get('duration')  # Already cleaned as a timedelta
        doctor = self.initial.get('doctor')  # Doctor must be passed during form initialization

        if not date or not time_str or not duration or not doctor:
            return cleaned_data  # Skip validation if critical fields are missing
